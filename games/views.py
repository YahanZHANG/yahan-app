...
def get_ranking_data(user, game, level):
    ...
    personal_score = base_queryset.filter(user=user).first()
    if personal_score is None:
        personal_best = None
        personal_rank = None
    else:
        personal_best = personal_score.score
        better_score_count = GameScore.objects.filter(
            game=game,
            level=level,
            score__gt=personal_best,
        ).count()
        personal_rank = better_score_count + 1
...
def block_breaker(request):
    ...
    return render(request, "games/block_breaker/index.html", context)


@login_required
def tap_star(request):
    ranking = get_ranking_data(request.user, GameScore.Game.TAP_STAR, 1)
    context = {
        "ranking_entries": ranking["entries"],
        "personal_best": ranking["personal_best"],
        "personal_rank": ranking["personal_rank"],
    }
    return render(request, "games/tap_star/index.html", context)


@login_required
def maze_chase(request):
    ranking = get_ranking_data(request.user, GameScore.Game.MAZE_CHASE, 1)
    context = {
        "ranking_entries": ranking["entries"],
        "personal_best": ranking["personal_best"],
        "personal_rank": ranking["personal_rank"],
    }
    return render(request, "games/maze_chase/index.html", context)
...
def save_score(request):
    ...
    level = normalise_level(game, data.get("level"))
    if level is None:
    ...
    with transaction.atomic():
        ...
        is_new_best = created
        if not created and score > game_score.score:
            game_score.score = score
            game_score.save()
            is_new_best = True

    ranking = get_ranking_data(request.user, game, level)
    return JsonResponse({
        "ok": True,
...
def ranking(request, game):
    ...
    level = normalise_level(game, request.GET.get("level"))
    if level is None:
        return JsonResponse({"ok": False, "error": "Invalid level."}, status=400)
